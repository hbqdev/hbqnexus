...
import QuoteRotator from '@/components/home/QuoteRotator.vue'
...
// or the quote back into App.vue outside router-view - this asserts
// against exactly that, for BOTH components, not just the banner.
//
// The quote now lives in QuoteRotator, inside HeroBanner, inside HomeView.
// It is stubbed: unstubbed, its onMounted hook fetches '/api/random-quote',
// and happy-dom's window.location is http://localhost:3000 - the exact port
// the live hbqnexus-api.service listens on. Without this stub `npm test`
// fires real requests at production. A stub still renders as a placeholder
// element, so findComponent(QuoteRotator) can still assert its
// presence/absence - the stub doesn't make the assertion vacuous.
describe('banner and quote are home-only', () => {
  ...
  it('does not render the banner or quote on a non-home route', async () => {
    ...
    const wrapper = mount(App, {
      global: {
        plugins: [router],
        stubs: { AboutModal: true, ContactModal: true, QuoteRotator: true },
      },
    })

    expect(wrapper.find('[data-test="bar"]').exists()).toBe(false)
    expect(wrapper.findComponent(QuoteRotator).exists()).toBe(false)
  })
  ...
  it('renders the banner and quote on the home route', async () => {
    ...
    expect(wrapper.findComponent(QuoteRotator).exists()).toBe(true)
  })
})
